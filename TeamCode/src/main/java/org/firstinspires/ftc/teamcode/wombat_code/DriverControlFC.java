...
public class DriverControlFC extends LinearOpMode {
    ...
    private final double MAX_CLAW = 0.25;
        ...
        while (opModeIsActive()) { // Runs on PLAY press. This section handles all player inputs.
            ...
            if (!presetActive) {
                if (gamepad2.x) {
                    ...
                    clawPos += (gamepad2.dpad_up && clawPos < MAX_CLAW) ? 2 * SERVO_INCREMENT : ((gamepad2.dpad_down && clawPos > 0) ? -2 * SERVO_INCREMENT : 0);
            ...
            telemetry.addData("Slider Left", motorSliderLeft.getTargetPosition());
            telemetry.addData("Slider Right", motorSliderRight.getTargetPosition());
            telemetry.addData("Base Left", baseLeftPos);
            telemetry.addData("Base Right", baseRightPos);
