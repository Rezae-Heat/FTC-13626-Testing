...
public class AutonomousMovementTest extends LinearOpMode {
    ...
    IMU imu;
    DcMotor motorIntake;
    DcMotorEx motorSliderLeft, motorSliderRight, motorFrontLeft, motorFrontRight, motorBackLeft, motorBackRight;
    Servo servoClaw, servoPitch, servoBaseLeft, servoBaseRight;

    boolean parked;
    int motorBLPos, motorBRPos, motorFLPos, motorFRPos, detectTag = 1;
    double clawPos = 0.22, pitchPos = 0.55, baseLeftPos = 1.0, baseRightPos = 1.0 - baseLeftPos;
    final double WHEEL_CIRCUMFERENCE = Math.PI * 0.096, TICKS_PER_REVOLUTION = 537.7;
    ...
    private void moveToAprilTag(int tag) throws InterruptedException {
        List<AprilTagDetection> currentDetections = aprilTag.getDetections();

        for (AprilTagDetection detection : currentDetections) {
            if (detection.id == tag) {
                drive(1, 0, 0, detection.ftcPose.x);
                driveToPos.start();
                driveToPos.join();

                drive(0, 1, 0, detection.ftcPose.y - 0.2);
                driveToPos.start();
                driveToPos.join();
